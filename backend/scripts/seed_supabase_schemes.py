...
logger = logging.getLogger("supabase_seeder")

try:
    from app.services.rag import rag_service
    from app.models.ingest import BaseSchemeIngest
    logger.info("Successfully imported core services and models.")
...
SCHEMES_TO_SEED = [
    ...
    BaseSchemeIngest(
        ...
        description_te=(
            "ఆంధ్రప్రదేశ్ సామాజిక రక్షణ పెన్షన్ పథకం ద్వారా వృద్ధులు, వితంతువులు, ఒంటరి మహిళలు మరియు చేనేత కార్మికులకు సామాజిక భద్రత కల్పిస్తూ ప్రతి నెలా పింఛను సరఫరా చేస్తారు."
        ),
...
def seed_database():
    logger.info("=============================================================")
    logger.info("       SEEDING AP & TS WELFARE SCHEMES TO SUPABASE           ")
    logger.info("=============================================================")
    
    # Empty pre-existing values if configured to maintain a perfect sync
    try:
        supabase = rag_service.supabase
        logger.info("Sanitizing destination DB schemas for precise overwriting...")
        for sc in SCHEMES_TO_SEED:
            supabase.table("schemes").delete().eq("name", sc.name).execute()
        logger.info("Sanitation complete.")
    ...
    for idx, sc_item in enumerate(SCHEMES_TO_SEED):
        logger.info(f"[{idx+1}/{len(SCHEMES_TO_SEED)}] Ingesting policy: '{sc_item.name}'...")
        try:
            res = rag_service.ingest_scheme(sc_item)
            logger.info(f"   ✔ Implemented successfully! ID: {res['scheme_id']} | Chunks Created: {res['chunks_count']}")
            success_count += 1
        except Exception as err:
            logger.error(f"   ❌ Errored while processing policy '{sc_item.name}': {str(err)}")
            fail_count += 1

    logger.info("=============================================================")
    logger.info(f" SEEDING SUMMARY: {success_count} succeeded, {fail_count} failed.")
    logger.info("=============================================================")

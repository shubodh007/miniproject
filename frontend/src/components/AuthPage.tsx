...
export const AuthPage: React.FC<AuthPageProps> = ({ onLogin }) => {
  ...
  // Helper to generate SHA-256 hash of direct inputs using Web Crypto API
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const invalidCredentialsMsg = language === 'te' ? 'చెల్లని ఇమెయిల్ లేదా పాస్‌వర్డ్' : 'Invalid email or password';
    ...
    if (activeTab === 'signin') {
      try {
        ...
        if (matched || isDemoCitizen || isDemoGuest) {
          ...
          sessionStorage.removeItem('sc_redirect_intent');
          navigate(storedIntent || '/dashboard');
        } else {
          // Prevent account enumeration by returning a generic credential validation error
          setErrors({ global: invalidCredentialsMsg });
          toast.error(invalidCredentialsMsg);
        }
      } catch (err) {
        setErrors({ global: 'Authentication engine error. Please check values.' });
        toast.error('Authentication engine error. Please check values.');
      }
    } else {
      // Sign up flows
      try {
        ...
        const newUser = { name: fullName, email, passwordHash: pwdHash }; // Never store raw plain-text password
        users.push(newUser);
  ...
  const handleGoogleLogin = async () => {
    // NOTE: This Google OAuth flow requires the Supabase Auth provider to be configured
    // and enabled in the Supabase dashboard.
    try {
      ...
      setErrors({ global: 'Failed to initiate Google Login.' });
    }
  };
  ...
  React.useEffect(() => {
    ...
    try {
      ...
      console.warn('Security cleanup failed on startup:', e);
    }
    ...
    const handleMessage = (event: MessageEvent) => {
      ...
      if (event.data?.type === 'OAUTH_AUTH_SUCCESS') {
        ...
        setErrors({ global: event.data.error || 'Google login failed.' });
      }
    };
    ...
    <div className="min-h-screen pt-28 pb-16 flex items-center justify-center px-4" id="auth-viewport">
      <div className="w-full max-w-4xl bg-bg-surface border border-border-main rounded-3xl overflow-hidden shadow-2xl flex flex-col md:flex-row animate-in fade-in duration-500" id="auth-card">
        ...
        <div className="w-full md:w-1/2 p-6 sm:p-8 flex flex-col justify-center">
          ...
          {isSuccess && (
            <div className="bg-success/10 border border-success/20 p-4 rounded-xl flex items-start space-x-2 text-success text-xs font-bold mb-6">
